...
1. **3C.2 状态（四维）：implementation completed / automated tests completed / live acceptance = not required（不开放 Evidence HTTP 端点）；real_evidence_extractor_smoke = completed（2026-08-09，真实 DeepSeek V4 Flash smoke 走生产路径通过，见 §13）**。
...
3. **LLM 抽象（Protocol）**：backend 无现有 ChatModel/factory → 最小 `EvidenceExtractionModel` Protocol（`@runtime_checkable`）：`model_id: str` + `async extract(research_question, retrieval_hit) -> EvidenceExtractionDecision`。domain/service 不直接依赖具体 DeepSeek/OpenAI provider。自动测试一律用 `FakeEvidenceExtractionModel`（`tests/evidence/fakes.py`）。`temperature=0`；禁止 tools / web search / function side effects。
   - 生产 adapter `DeepSeekEvidenceExtractionModel`（`app/evidence/extractor/adapters.py`）：lazy import `langchain_deepseek.ChatDeepSeek`（构造 adapter / 模块导入不依赖 langchain 已安装；调用 `extract()` 时才加载）；`model_id` = `provider:model`（`deepseek:deepseek-v4-flash`，绝不伪造 revision）；`with_structured_output(EvidenceExtractionDecision)`（默认 function_calling 方法）；`temperature=0`、`timeout` / `max_retries` 来自 Settings；**显式 `extra_body={"thinking": {"type": "disabled"}}` 关闭 thinking**。provider 名由调用方配置（factory 分派），不写死。
...
13. **真实 LLM smoke（§13，2026-08-09 completed）**：最多执行一次、人工构造非敏感 chunk（"公司2025年营业收入为100亿元，同比增长12%。"）+ 固定 research question；**必须走生产路径**（Settings → LLM Factory → DeepSeekEvidenceExtractionModel → ChatDeepSeek → EvidenceExtractionDecision），不写临时 DeepSeek SDK 请求绕过项目代码。
    - **结果**：`real_evidence_extractor_smoke = completed`。真实调用 1 次成功（无重试）：provider=deepseek、request_model=deepseek-v4-flash、thinking 显式 disabled、latency_ms≈8875、relevant=true、item_count=1、evidence_type=metric、confidence=high、`resolve_exact_quote` 成功得到 [start, end)。**不写正式 evidence_cards、不持久化、不记录 API key / 完整 prompt / reasoning_content / provider raw response**。
    - 认证失败立即停止不重试；临时 network/rate-limit/timeout 最多额外重试 1 次（本 smoke 未触发）。真实 LLM 只用于一次性受控 smoke，不进入自动化测试（自动测试用 FakeEvidenceExtractionModel）。
    - **模型迁移（2026-08-09）**：DeepSeek 官方已停止 legacy model names（deepseek-chat / deepseek-reasoner）；当前统一 `LLM_PROVIDER=deepseek`、`LLM_MODEL=deepseek-v4-flash`，model_id = `deepseek:deepseek-v4-flash`（不伪造 revision）。生产 adapter **显式传 `extra_body={"thinking": {"type": "disabled"}}`** 关闭 thinking（DeepSeek V4 Flash 默认 thinking，但结构化抽取需要稳定、低成本、无 reasoning_content 的输出；`temperature=0` 不等于关闭 thinking）。`thinking` 非标准 OpenAI 参数，按 langchain-deepseek==1.1.0 公开接口经 `extra_body` 传递（库文档明确：非 OpenAI 自定义参数必须走 `extra_body`，`model_kwargs` 会造成 API 错误）。**禁止记录 reasoning_content**。
