...
1. **2D.1 状态：implementation completed / automated tests completed / live external acceptance pending（2026-08-07）**。
   - News Discovery 基础 + GDELT DOC 2.0 Discovery Provider 已实现并冻结；75 项 News 单元测试（A. Contracts / B. GDELT Client / C. Parser / D. Fingerprint）+ 7 项 MockTransport E2E 集成测试全部通过；全量回归 731 项 non-integration + 110 项 integration（原 656/103）通过，ruff lint/format 通过，alembic = 0011 head。
   - 受控真实 Probe 已执行：本机对 `api.gdeltproject.org` 连接超时（ConnectTimeout，与 2C.1 对 `worldbank.org` 相同的域名级出口阻断环境），未重试、未代理、未 DNS 覆盖，acceptance 记录为 **pending**。跑通前不开放生产新闻发现、不把 Discovery Candidate 视为 Evidence、不开始 2D.2。
...
10. **GDELT JSON Parser（11 条规则）**。
    - 顶层必须 object（否则 `GdeltMalformedResponse`）；articles 缺失→空、必须是 list；单 article 必须 object（非 object 跳过）；缺 url/title 或 trim 空跳过；URL 非 http/https 跳过；seendate 无法解析跳过且**不代用当前时间**；domain 由 normalized URL hostname 派生、不盲信 Provider 字段；同一 normalized_url 去重保留第一条；rank 过滤后从 1 重排；输出顺序稳定。
...
14. **Discovery Persistence Service（discover_and_persist A-H）**。
    - A. provider.discover(query)（网络 I/O 期间绝不持有 AsyncSession）；B. 原始响应先 `put_json_bytes` 内容寻址落盘（文件 I/O 在 DB transaction 之前；孤儿文件保留等待后续 GC）；C. 短 DB transaction：RawArtifact get_or_create，media_type 必须 application/json，否则 `NewsDiscoveryArtifactConflict`；D. build_query_fingerprint（engine + company_id + query_text + UTC ISO 时间窗 + max_results + raw_content_sha256，不含 fetched_at/request_count/ID/storage_key）；E. run create-or-get by fingerprint（ON CONFLICT DO NOTHING + RETURNING，仅赢家 created=True）；F. replay 完整性检查：candidate_count == result_count，不符抛 `NewsDiscoveryIntegrityError`，不自动修复；G. 仅新 run 时 bulk insert Candidates（含 url_sha256、verification_status=unverified）；H. commit；DB 层异常 rollback 并抛 `NewsDiscoveryPersistenceFailed`。
...
20. **真实验收 pending 与数据门槛**。
    - 全部测试使用 MockTransport，原始归档只写测试临时目录，不连 Chroma；conftest autouse Network Guard 继续拦截任何非回环真实网络。受控 Probe 仅对 `api.gdeltproject.org` 一次请求、不重试（本环境连接超时 → acceptance pending）；没有任何持久化的真实 GDELT 数据。生产新闻发现在真实验收跑通前不开放。
...
- 新增测试 82 项：test_contracts.py（query 8 条校验 + URL normalization + candidate 规则）、test_client.py（固定 endpoint/参数、单次请求、无 Cookie/Auth、trust_env=false、redirect 安全、429/5xx/超时稳定错误、Content-Type、5 MiB、非法 JSON/NaN、日志字段白名单）、test_parser.py（11 条规则）、test_fingerprint.py（golden vector + 敏感性 + 时区归一化）、test_news_discovery_service.py 7 项 E2E（全链路持久化、replay 幂等、跨次字节稳定 replay、并发单 run、篡改候选 IntegrityError、DB 异常包装 PersistenceFailed、run 不产生 source_records、repository 查询辅助）。
- Migration 0011 已应用（`alembic current` = 0011 head）；downgrade 先删 candidates 再删 runs。
- 全量回归通过：731 项 non-integration + 110 项 integration；既有 PDF/Company/Task/Workflow/Macro 测试保持通过；ruff lint/format 通过。
- 遗留边界：2D.1 live external acceptance pending（网络阻断）；无任何持久化的真实 GDELT 数据；2D.2 / 2D.3 未开始。
