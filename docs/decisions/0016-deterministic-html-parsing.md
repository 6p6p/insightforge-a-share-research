...
4. **HTML parser（`app/parsing/html_parser.py`，`html_dom` VERSION=1，lxml 5.4.0 是唯一新增依赖）**。
   ...
   - **编码检测确定性（只从真实 meta 声明识别）**：`<meta charset="...">`（HTML5 属性形式）或 `<meta http-equiv="Content-Type" content="...; charset=...">`；**绝不全局扫描任意 `charset=` 文本**（body/script 内出现 `charset=...` 不影响编码判定）。流程是先 `_detect_encoding`（纯本地字节扫描，`raw[:8192]`，逐 `<meta>` 标签内查 charset 属性）把 bytes 解码为 str，再交 lxml（str 输入不再猜编码）。BOM → 声明 → UTF-8 默认（不依赖 libxml2 的 latin-1 猜测）。声明编码不可用（LookupError / UnicodeDecodeError）回退 UTF-8；无声明且非合法 UTF-8 → `HtmlParseError`（宁可失败，不可产出 latin-1 乱码文本）。**绝不显式给 lxml 写死 encoding**（显式 encoding 会覆盖 meta charset 声明，导致 GBK 页面被按 UTF-8 解错）。
