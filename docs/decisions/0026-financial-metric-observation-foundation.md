...
6. **Source value exactness（I，Gate 0 修正）**：`source_value_text`（trim 后）必须等于 `EvidenceCard.quote_text` 中**一个完整数字 token**。`find_financial_number_tokens`（`app/financial/number_parser.py`）扫描 quote_text 的全部完整数字 token，与 `parse_financial_number` **同一 grammar**：**0 个 → `FinancialMetricValueNotFound`、>1 个 → `FinancialMetricValueAmbiguous`**。**禁止 substring partial match**：`"收入1000万元"` 里 `"1000"` 是 token 而 `"100"` / `"000"` 不是；`"-123.45"` / `"(123.45)"` 的符号与括号属于 token，剥离后（`"123.45"`）不是 token。不做 fuzzy / normalize / 自动纠错 / LLM 修正；不信任 caller 提供的 raw_value / normalized_value_cny（draft 里根本不提供这两个字段）。

7. **确定性数字解析（J）**：`parse_financial_number`（`app/financial/number_parser.py`）用 `Decimal` 全程、**零 float**；v1 严格语法（可选千分位 + 可选小数 + 可选正负号/括号负号 + 首尾空白），**拒绝**科学计数法 / 百分号 / 中文数字 / 约数 / 带单位 / 畸形千分位 / 双小数点 / 括号不平衡 / 括号带符号 / 内部空白 / 非字符串。`scaleb` 处理小数位，`str(Decimal)` 保留 scale（指纹序列化语义一致）。parse 与 `find_financial_number_tokens` 共用同一 grammar（`source_value_text.strip()` 必须正好等于一个完整 token），保证 token 识别与数值解析永不漂移。
...
13. **测试（Gate 0 后）**：**82 项单元**（`tests/financial/test_contracts.py` 27：taxonomy 冻结 / family mapping / expected period kind / draft 输入防御 / fingerprint 确定性与敏感性 + `tests/financial/test_number_parser.py` 55：正例 8 + 反例 12 + normalize 9 + **tokenizer 15**（exact token / partial-match 拒绝 / 符号与括号属于 token / malformed 千分位 / 科学计数 / duplicate tokens）+ **storage bounds 6**，零 DB/零 LLM）+ **29 项集成**（`tests/integration/test_financial_metric_service.py`，真实 PG + 真实 HTML/PDF 服务链，零 Chroma/LLM：HTML / PDF 创建字段与换算、missing / wrong company / non-document origin / non-metric evidence 拒绝、value not found / ambiguous、**exact numeric-token 场景 7**（"1000" 接受 / "100" / "000" 拒绝 / 符号与括号属于 token / 两个完整 token ambiguous）、**storage overflow（unit normalize 后超 NUMERIC(38,12)）拒绝**、balance instant / duration requires period_start、replay 复用、**并发 → 1 行**、value / unit / period 变化 → 新行、**replay 篡改 raw_value / normalized_value → IntegrityError**、EvidenceCard 行永不修改、provenance 全链路回溯到 RawArtifact、精确阶段边界 `claims==2 / report 表==0`、Service 只持有 sessionmaker）+ **2 项 migration 0020 downgrade guard**（isolated 临时 PG：无数据降级成功、有 observation 数据降级拒绝且行保留）。全程 **0 LLM / 0 Chroma / 0 Claim / 0 Report**。全量测试在 Gate 0 提交前重跑确认，ruff 零告警，`pip check` 通过。
...
15. **Gate 0 修正与精度边界（2026-08-09）**：
    - **exact numeric-token provenance**（取代 substring count）：见第 6 条，token 与 parse 同一 grammar。
    - **NUMERIC(38,12) storage contract**：新增 `validate_financial_decimal_storage`（`app/financial/number_parser.py`）+ `FinancialMetricStorageRangeError`：raw_value 与 normalized_value_cny 落库前必须满足**小数位 <= 12 且 abs < 10^26**，不满足 → 显式拒绝（**禁止静默 quantize / round / truncate**，不让 PG 自动 rounding / overflow）。
    - **当前保证（已实现）**：Evidence / source provenance（Observation → EvidenceCard → Chunk → ParsedSource → SourceRecord → RawArtifact）、exact numeric-token provenance、Decimal parse（零 float）、period / type / unit 结构化校验（period_kind 与 metric family 对齐、scope / unit 枚举）、deterministic unit normalization。
    - **未声称（producer 输入边界）**：`metric_code` / `statement_scope` / `period` / `raw_unit` **当前仍由 structured producer 输入**（create_observation 的 draft），**尚未从年报自动识别验证**。不得宣称 4B.2A 已自动从 PDF / 年报抽取这些字段。未来 **official structured financial provider / deterministic table extractor** 可作为 producer（仍是确定性路径），届时再更新本节。
