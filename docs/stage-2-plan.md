...
- **2D.2A（状态四维：implementation = completed / automated_tests = completed / docker_rebuild_acceptance = completed / live_external_acceptance = completed，2026-08-07）**：Original Publisher Verification + Safe HTML RawArtifact archival + SourceRecord 登记。
  ...
  - 测试：**45 项新单元测试**（Resolver 17 / SafeHtmlFetcher 28）+ **9 项 MockTransport E2E 集成测试**（全链路/replay/并发去重/共享 SourceRecord/Network Guard/无 Evidence 结构不变量/错误路径）+ **3 项协议与服务边界回归**；全套 784 非集成 + 123 集成测试通过，ruff 零告警，`pip check` 通过。
  - 受控真实 Probe（§二十八）：对 `www.xinhuanet.com` **恰好 1 次请求**、不重试、不代理、不 DNS 覆盖；真实 DNS + 真实 HTTPS → status=200 / text/html / redirects=0 / 178,458 字节，**live external acceptance = completed**。Docker 重建成功（新镜像 healthy、live 200、ready 200、五项 checks ok），docker_rebuild acceptance = completed。
...
- **2D.2B（尚未开始）**：verification status 演进（rejected / archived / evidence_ready）与真实新闻正文进入 Evidence 管线。
