...
- **2D.2A（状态四维：implementation = completed / automated_tests = completed / docker_rebuild_acceptance = completed / live_html_transport_acceptance = completed，2026-08-07）**：Original Publisher Verification + Safe HTML RawArtifact archival + SourceRecord 登记。real_article_e2e_acceptance = not performed / not required for 2D.2A code acceptance（真实文章端到端由自动化测试覆盖：真实 PostgreSQL + MockTransport + FakeHostResolver）。
  - 确定性链路：`NewsDiscoveryCandidate → Original Publisher（OriginalPublisherResolver）→ Safe HTML fetch（SafeHtmlFetcher，DNS/SSRF + 域名归属 + 手动重定向重校验）→ RawArtifact(text/html) 内容寻址归档 → SourceRecord(news_article/public_html) → NewsSourceVerification`。`verified` 语义严格限定为"发布者归属 + 页面可访问 + 原文已归档 + 溯源已建立"，不是内容真实性判定（后续 Evidence 管线）。
  - 不变量 A-H 冻结（Discovery Provider ≠ Source Provider；Candidate URL 不直接请求；仅 enabled+news_article+public_html 有资格；seen_at 永不是 published_at；SourceRecord.provider_key 必须是 xinhuanet/cnstock/cs_com_cn；source_url 用 final URL）。
  - migration 0012（已应用）：`news_source_verifications` 新表 + 5 个 CHECK 约束演进（provider_type +media、media_type +text/html、document_type +news_article、acquisition_method +public_html、verification_status +verified）；downgrade 0011 / 再 upgrade 往返验证通过。
  - replay 零网络返回 replayed=true；SourceRecord 去重键 `(provider_key, final_url, artifact_id)`，多 Candidate 共享 SourceRecord、各一条 Verification；并发 `ON CONFLICT DO NOTHING + RETURNING`，无进程锁。稳定错误 8 类；news_article 只能经本服务产生（upload/import-url 拒绝）；`GET /source-records/{id}/content` 对 HTML 返回 415。
  - 测试：**45 项新单元测试**（Resolver 17 / SafeHtmlFetcher 28）+ **9 项 MockTransport E2E 集成测试**（全链路/replay/并发去重/共享 SourceRecord/Network Guard/无 Evidence 结构不变量/错误路径）+ **3 项协议与服务边界回归** + **4 项一致性收口测试**（DomainError 415 契约冻结 ×1、真实链路 HTML 415 + XSS 防护 + stream 生命周期 ×2、Candidate verification_status 冻结 ×1，后 3 项走真实 PostgreSQL + 真实 LocalRawArtifactStore + 真实 FastAPI）；全套 **785 非集成 + 126 集成测试通过**，ruff 零告警，`pip check` 通过。
...
- **2E.2（completed，2026-08-07）**：确定性 PDF 解析 + page location。
  ...
  - `pdf_layout` parser（VERSION=2，pdfplumber 0.11.10 + lxml 5.4.0 锁定精确版本）：只读 bytes（内部 BytesIO），不联网、不修改 PDF、不写临时外部文件；仅支持 machine-generated PDF，整个 PDF 无文本 → `PdfTextUnavailable`（OCR 留未来），单页无文字不失败。
  - 确定性提取：每页 `dedupe_chars()`（仅去同一坐标重复绘制字符）→ `extract_words(use_text_flow=False, keep_blank_chars=False, expand_ligatures=True, 固定 x/y tolerance)`（不用 experimental API）；固定排序 page ASC → top ASC → x0 ASC；固定 y tolerance 聚行，每行一个 block（`block_type=paragraph`，不推断 heading/语义）；空文本跳过。**不做 text-level 去重（2E.2 收口，v1→v2）**：同页不同 bbox / 跨页的相同文本行全部保留，由 pdf_page locator 区分原文位置。
