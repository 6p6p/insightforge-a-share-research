...
- **后续**：4B.1（Structured Claim Analysis Foundation + Business / Event / Risk）= completed（见下节）；**4B.2A（Financial Metric Observation Foundation）= completed（见下节）**；**4B.2B（Deterministic Financial Calculation）= completed（见下节）**；**4B.2C.1（Financial Claim Provenance）= completed（见下节）**；**4B.2C.2（Structured Financial Analysis）= completed（见下节）**；4C.1（Macro Context Analyst）= FINAL（4C.1A + 4C.1B，见下节）；4C.2A（Relative Valuation Comparison Foundation）= completed（见下节）；4C.2B.1（Relative Valuation Claim Provenance）= completed（见下节）；4C.2B.2（Relative Valuation Claim 分析）= next；4D（Claim 综合 / 冲突 / 证据缺口）= later；**Report 生成与 Agent Audit 属于 Stage 5**，不提前标记。Financial 统一归属 4B.2，4C 不重复 Financial。详见 [docs/stage-4-plan.md](docs/stage-4-plan.md)。
...
- **边界**：不实现 4C.2B Relative Valuation Claim（4C.2A 已 completed，见下节）；不实现 Claim Synthesis / Conflict / Evidence Gap（4D）；不接 LangGraph 分析节点（Service 层供未来 LangGraph 顶层编排调用）；不开放 HTTP API；不生成 Report / DraftSection / ReviewIssue / Audit；不改动 generic Claim schema；不批量 update 历史 rows；**不开始 4C.2B**。
- **决策记录**：[docs/decisions/0032-structured-macro-context-analyst.md](docs/decisions/0032-structured-macro-context-analyst.md)。

## Relative Valuation Comparison Foundation（阶段 4C.2A）

阶段 4C.2A 建立相对估值的数据与比较基础：**Source → EvidenceCard(metric) → ValuationMetricObservation → RelativeValuationComparison**，为 4C.2B（Relative Valuation Claim）提供可审计的数值地基。**状态（2026-08-10）：implementation completed / automated tests completed / live acceptance not required（不开放 Valuation HTTP 端点）**。migration **0026**（`valuation_metric_observations` + `relative_valuation_comparisons` + `relative_valuation_comparison_peers`，全列带 CHECK / UNIQUE / INDEX；downgrade guard：**任一表有行 → 拒绝回滚**，alembic_version 保持 0026）。版本边界：**当前 `VALUATION_OBSERVATION_SCHEMA_VERSION=1` / `RELATIVE_VALUATION_COMPARISON_SCHEMA_VERSION=1` / `VALUATION_FORMULA_VERSION=1`**；comparison_method v1 = `peer_median`；**v1 metric_code 只有 `pe_ttm` / `pb_mrq` / `ps_ttm`**。核心边界：Observation = 绑定真实 metric EvidenceCard 的**原始估值倍数事实**（允许 0 / 负倍数作为来源快照）；Comparison = 对显式 peer 集合计算的确定性派生比较事实（peer_median / min / max / premium_discount_to_median），**不是 EvidenceCard**；**不做** DCF / PEG / EV / EBITDA / FCFF / FCFE / target price / dividend model / 绝对公允价值 / 分类（分类在 4C.2B）。

- **包位置**：`app/valuation/`（contracts / errors / number_parser / observation_service / comparison_service）+ 3 个 repository + 3 个 DB model。**纯函数复用**：`find_financial_number_tokens` / `fits_numeric_38_12`（领域无关 grammar）、`resolve_availability`（no-lookahead）。
- **Observation evidence policy（v1 严格）**：bind 真实 EvidenceCard；`evidence.company_id == draft.company_id`；`origin_type=document_chunk` 且 `evidence_type=metric`；任何不匹配 → `ValuationObservationEvidenceMismatch`。**source_value_text 必须是 quote_text 中一个完整数字 token**：先 grammar parse（`parse_valuation_number`，`"100万"` → `ValuationValueNotNumeric`），再 exact-match（0 个 → `ValuationValueNotFound`，>1 个 → `ValuationValueAmbiguous`）；禁止 substring partial match / fuzzy / normalize 后匹配 / 自动纠错（`"市盈率30倍"` 里 `"30"` 接受而 `"3"` / `"0"` 拒绝）。`metric_value` 全程 `Decimal`、零 float，通过 `validate_valuation_decimal_storage`（NUMERIC(38,12) 无失真；超界 → `ValuationStorageRangeError`）。
- **当前保证 / producer 输入边界**：**已实现（deterministic）** = Evidence/source provenance（company / document metric Evidence / origin_type / evidence_type）、exact numeric-token provenance（`source_value_text` ↔ `quote_text` 完整 token）、`Decimal` parse、storage bounds、同 metric/date 可比较性、确定性统计与 fingerprint/replay。**尚未实现（structured producer semantics）** = 从 quote / 表格表头自动识别或验证 `metric_code`（该数字是否 `pe_ttm` / `pb_mrq` / `ps_ttm`）与 `metric_as_of`（该数字的市场观测日）——这两个字段当前由 structured producer（`ValuationMetricDraft`）声明，程序只确定性校验其**取值范围**（metric_code 白名单 / metric_as_of 为 date）与跨 observation 一致性，**不自动从原文语义推断**"所有估值语义已从原文自动验证"。未来 official market-data provider / deterministic table extractor 可作为 producer 自动填充这两个字段（仍是确定性路径）。本阶段**不实现**中文 PE/PB/PS label NLP parser。
- **Peer 规则（显式 peer，程序不自动选）**：调用方传 `peer_observation_ids`（3..20）；Service 确定性校验：缺失 → `ValuationObservationNotFound` / `ValuationCompanyNotFound`；target 公司 != draft → `ValuationCompanyMismatch`；peer 公司重复 → `ValuationPeerDuplicateError`；peer 含 target → `ValuationPeerIncludesTargetError`。
- **可比较性 / no-lookahead**：同一集合全部同一 metric_code（`ValuationMetricMismatch`）/ 同一 metric_as_of（`ValuationDateMismatch`，严格 same-date）；任一 metric_value <= 0 → `ValuationMetricNotComparable`；`analysis_as_of >= metric_as_of` 且每个来源 availability（published_at 否则 acquired_at，绝不用 reporting_period_end）<= analysis_as_of，未来 → `ValuationFutureEvidence`。
- **确定性统计（纯函数，全程 Decimal）**：`compute_peer_median`（奇数 → 中间值；偶数 → 两中位算术平均）；`compute_comparison_stats` → median / min / max / `premium = (target - median) / median`；`CALCULATION_SCALE=12`、`ROUND_HALF_EVEN` 显式局部上下文；中位数恒有限小数，ROUND_HALF_EVEN 只作用于 premium 除法（如 -0.2/15.5 → -0.012903225806）。
- **Fingerprint + replay**：canonical JSON + SHA-256；comparison fingerprint 的 peer list **按 peer_company_id 排序**、含 target/peer observation fingerprint；**均不含 comparison_id / created_at**。`create_or_get`（ON CONFLICT DO NOTHING RETURNING，无进程锁）；replay 重新加载真实 DB 逐项核实，损坏 → `ValuationIntegrityError`，不自动 repair（修改 = 新行，无 update API）；SQLAlchemyError → 整批 rollback + `ValuationPersistenceFailed`（0 partial write）；并发 → 最终 1 行 + 1 套完整 peer links。Service 只持有 sessionmaker。
- **测试**：**9 项单元**（`tests/valuation/test_valuation_stats.py`）+ **27 项集成 observation**（`tests/integration/test_valuation_observation_service.py`）+ **22 项集成 comparison**（`tests/integration/test_valuation_comparison_service.py`）+ **3 项 migration 0026 downgrade guard**（`tests/integration/test_migration_0026_downgrade_guard.py`，isolated 临时 PG：空库降级成功 / 有 observation 拒绝 / 有 comparison+peers 拒绝且数据保留）。全程 0 真实 LLM / 0 Chroma / 0 LangGraph / 0 Claim / 0 Report 表。
- **边界**：4C.2B.1 Relative Valuation Claim Provenance 已 completed（见下节）；不开始 4C.2B.2（LLM 分析）；不做估值分类 / 绝对公允价值 / target price / DCF / PEG / EV / EBITDA / FCFF / FCFE / dividend model；不自动选 peer；不修改 0023 / 0024 / 0025 / 0026；不批量 update 历史 rows；不反推历史 cutoff；不接 LangGraph 分析节点；不开放 HTTP API；不创建 Claim / Report / DraftSection / ReviewIssue / Audit。
- **决策记录**：[docs/decisions/0033-relative-valuation-comparison-foundation.md](docs/decisions/0033-relative-valuation-comparison-foundation.md)。

## Relative Valuation Claim Provenance（阶段 4C.2B.1）

阶段 4C.2B.1 把引用**已登记 `RelativeValuationComparison`** 的 Relative Valuation Claim 确定性登记为 Claim + RelativeValuationClaimProfile + ClaimRelativeValuationComparisonLink + 自动展开的 Evidence links，形成 **Claim → ClaimRelativeValuationComparisonLink → RelativeValuationComparison → ValuationMetricObservation → EvidenceCard → Source** 完整可重算证据链。**状态（2026-08-10）：implementation completed / automated tests completed / live acceptance not required（不开放 Valuation Claim HTTP 端点）**。migration **0027**（`claim_relative_valuation_comparison_links` + `relative_valuation_claim_profiles`，全列 CHECK / UNIQUE / INDEX；downgrade guard：**任一表有行 → 拒绝回滚**，alembic_version 保持 0027）。版本边界：**当前 `VALUATION_CLAIM_SCHEMA_VERSION=7` / `VALUATION_CLAIM_PROFILE_SCHEMA_VERSION=1`**（不改 generic Claim v1 / Financial v2/v3 / Macro v4/v5/v6）。**0 LLM / 0 Chroma / 0 Retrieval / 0 LangGraph / 0 Report / 0 Audit**。

- **包位置**：`app/valuation/`（claim_contracts / claim_errors / claim_service）+ `app/valuation/comparison_service.py`（新增 public `verify_comparison_integrity` shared helper）+ 2 个 repository + 2 个 DB model + migration 0027。
- **ValuationClaimDraft（专用新类）**：只允许语义输入；固定 `analysis_domain=valuation`、`claim_kind=relative_valuation`（draft 不提供，Service 强制）；**至少 1 个 support_comparison_id**；v1 最多 3 个 comparison（PE / PB / PS 各最多 1 个）；同一 comparison / Evidence 不能跨 relation 重复；`assessment` 是**分析判断**（无 hidden thresholds、不从 premium 自动推导）。
- **Comparison integrity（spec I）**：通过 shared helper `verify_comparison_integrity` 复用真实 `RelativeValuationComparisonService`（不复制 formula / replay logic）。comparison 缺失 → `ValuationClaimComparisonNotFound`；跨公司 → `ValuationClaimComparisonMismatch`；analysis_as_of 不一致 → `ValuationClaimAnalysisDateMismatch`；metric_as_of 不一致 → `ValuationClaimMetricDateMismatch`；peer_company_id 集合不一致 → `ValuationClaimPeerSetMismatch`；metric_code 重复 → `ValuationClaimDuplicateMetric`；重放损坏 → `ValuationClaimIntegrityError`（**不 repair**）。
- **Relation semantics / Evidence expansion**：relation 由 draft 显式指定（**程序不根据 premium 自动决定**）；每个 comparison → target + 全部 peer Observations 的 source EvidenceCards 自动加入 ClaimEvidenceLinks（**一律 relation=context**，跨 comparison 幂等去重）；additional Evidence 保持 caller 指定 relation（跨公司 → `ValuationClaimEvidenceCompanyMismatch`；与自动 context 冲突 → `ValuationClaimRelationConflict`，不静默选一个）。
- **Critical policy（spec Q）**：critical Claim 要求每个 support Comparison 的 target + peer 全部 source Evidence `critical_claim_eligible_snapshot=true`（→ `ValuationClaimCriticalEvidenceInsufficient`）；additional supports 不能替代。
- **Fingerprint + replay（spec R/U）**：v7 canonical JSON + SHA-256，含 claim_schema_version=7 / profile_schema_version=1 / semantic fields / comparison groups（comparison_id + comparison_fingerprint）/ evidence groups（evidence_card_id + evidence_fingerprint）；**不含 claim_id / created_at**。replay 重新加载全部行重新派生逐项核实，损坏 → `ValuationClaimIntegrityError`，不自动 repair（修改 = 新 Claim，无 update API）。
- **Persistence / batch（spec S/T）**：two-phase commit（短 session 加载校验 → 关闭 → 纯函数派生 → 短 transaction create_or_get + links），0 partial write；并发 → 1 Claim + 1 Profile + 1 套 links（ON CONFLICT，无进程锁）；`create_claim_batch` 1..3 all-drafts-validate-first + 单 transaction（任一失败 → 整批 rollback）；items 按 input order。Service 只持有 sessionmaker。
- **测试**：**34 项集成**（`tests/integration/test_valuation_claim_service.py`）+ **3 项 migration 0027 downgrade guard**（`tests/integration/test_migration_0027_downgrade_guard.py`，isolated 临时 PG：空库降级成功 / profiles 行拒绝 / links 行拒绝且数据保留）。全程 0 真实 LLM / 0 Chroma / 0 LangGraph / 0 Report 表。
- **边界**：不开始 4C.2B.2（LLM 分析）；不调用 LLM / 不调 DeepSeek / 不查 Chroma / 0 Retrieval；不创建 Report / DraftSection / ReviewIssue / Audit；不接 LangGraph 分析节点；不开放 HTTP API；不修改 generic / Financial / Macro 既有 schema；不实现绝对公允价值 / target price / DCF / 买卖建议；**不把 `RelativeValuationComparison` 伪装成 EvidenceCard**。
- **决策记录**：[docs/decisions/0034-relative-valuation-claim-provenance.md](docs/decisions/0034-relative-valuation-claim-provenance.md)。

## Structured Relative Valuation Analysis（阶段 4C.2B.2）

阶段 4C.2B.2 把 4C.2B.1 的 Comparison + Claim provenance **接上 LLM 分析判断**，形成 Structured Relative Valuation Analyst：**Analyst 只做判断，确定性交给代码**。调用方传 `RelativeValuationComparison[] + research question + analysis_as_of`（`ValuationAnalysisRequest`，comparison_ids 1..3，**不接 additional Evidence**）→ DeepSeek 结构化决策（`ValuationAnalysisDecision`：relevant / assessment / confidence / importance / 每条 comparison 归入 supports·contradicts·context）→ 确定性校验 + 确定性 statement 渲染 → `ValuationClaimDraft(schema v7)` → `ValuationClaimService` → Relative Valuation Claim。**状态（2026-08-10）：implementation completed / automated tests completed / 4C.2B.2 runtime acceptance completed（Windows + Docker 各 live/ready 5×200、五项 checks ok；Docker runtime `alembic_version=0027` + 三张 valuation 表可读；`VALUATION_ANALYST_VERSION=2`，v1=historical pre-final，v2=current）**。**无新 migration（Alembic head 保持 0027）**。

- **包位置**：`app/analysis/valuation/`（contracts / decision / packs / prompt / model / adapters / factory / errors / service）+ `app/valuation/claim_policy.py`（共享确定性策略单例）。
- **角色边界（spec B）**：LLM **不**计算 median / premium / percent、不选择 peers、不生成任何数值、不生成 Claim statement、不生成 target price / fair value / 交易建议。确定性代码负责：V alias（V1..Vn 按 metric_code pe_ttm/pb_mrq/ps_ttm 排序）、ref resolution（V → comparison_id）、unknown ref / cross-relation / omitted input（**no-cherry-picking 硬边界**，support∪contradict∪context 恰好等于全部 input aliases）拒绝、direction consistency（relative_high 全正 / relative_low 全负 / mixed 正负都有 / broadly_in_line 无 threshold，**无 hidden thresholds**）、uncertain importance policy（uncertain→normal）、`render_valuation_claim_statement(assessment, metric_codes)` 冻结中文 statement 渲染（v2：metric_codes 来自实际 selected verified Comparisons，single PE/PB/PS 与 multi 综合文本；不含数字/百分比/公司/peer 名称）、v7 Claim 创建 / replay。
- **共享策略单例（spec I）**：跨 comparison 一致性 + direction / uncertain 策略集中在 `app/valuation/claim_policy.py`（`ValuationClaimPolicyReason` 8 个稳定 reason），ValuationClaimService 与 ValuationAnalysisService **共用**，禁止复制两套规则；`_policy_to_analysis_error` 映射到 analysis 错误域。
- **Model boundary（spec H）**：`ValuationAnalysisModel` Protocol 独立 `model.py`；生产适配器 `DeepSeekValuationAnalysisModel` = 懒加载 ChatDeepSeek（model_id=`deepseek:deepseek-v4-flash`）+ temperature=0 + **显式关闭 thinking**（`extra_body={"thinking": {"type": "disabled"}}`，不产生 reasoning_content）+ with_structured_output（不绑定 tools / web search）；OutputParserException → MalformedOutput、其余 → ModelUnavailable。
- **Prompt 契约**：system 冻结（`VALUATION_ANALYSIS_SYSTEM_PROMPT`）+ Comparison 用 `<<<COMPARISON_DATA_START/END>>>` delimiter 只进 **user** payload（trusted derived data，绝不拼进 system）；不发送 comparison/observation/Evidence UUID、fingerprint、locator、Chroma metadata。
- **Pack 最小投影**：每个 V item 只含 valuation_ref / metric_code / target_value / peer_median / peer_min / peer_max / premium_discount_to_median / position_vs_median（程序判定）/ peer_count / metric_as_of / analysis_as_of / comparison_method / formula_version / deterministic_display_premium（程序生成）；`_decimal_str` 规范化 DB numeric(14,12) 尾零（如 15.300000000000 → "15.3"），单测直接构造与 DB 读出渲染一致。
- **流程（spec K，两步提交镜像 FinancialAnalysisService）**：短 session 加载 Comparison 并逐条 `verify_comparison_integrity`（缺失 / 跨公司 / 损坏 → 稳定错误，**不调用 LLM**）→ 关闭 session（LLM 调用不持有 DB connection）→ Pack → 调模型 → relevant=false 0 写 → ref resolution → direction / uncertain 策略 → 确定性 statement + draft → `ValuationClaimService.create_claim` 原子登记；任一失败 → **整次 0 写**。
- **错误分类（spec J）**：`app/analysis/valuation/errors.py` 14 个稳定错误类（ValuationAnalysisError 基类 + InputError / ComparisonNotFound / ComparisonCompanyMismatch / ComparisonCorrupted / InputInvalid / MalformedOutput / ModelUnavailable / UnknownRef / RelationConflict / ComparisonOmitted / DirectionConflict / MixedEvidenceInsufficient / UncertainImportancePolicy / ClaimDraftError）。错误消息不包含 evidence 正文 / raw provider response / API key / 完整 prompt / DB URL / UUID alias 映射。
- **测试（46 项单元 + 18 项集成 + Gate 0 4 项）**：单元 `tests/analysis/valuation/`（test_packs 13 / test_contracts 23 / test_prompt 10，零 DB + fake）；集成 `tests/integration/test_valuation_analysis_service.py`（happy path v7 claim / broadly_in_line / 最小 pack 投影 / V alias 排序 / relevant=false / comparison missing·跨公司·corrupted 不调用 LLM / unknown ref·cross-relation·omitted·direction conflict·mixed insufficient·uncertain importance·malformed·model unavailable 0 写 / replay 同 claim）；Gate 0 在 `tests/integration/test_valuation_comparison_service.py`（verify_comparison_integrity：missing→None、损坏→ValuationIntegrityError 不泄漏 input error）。全程 0 真实 LLM / 0 Chroma / 0 LangGraph / 0 Report 表。
- **真实 DeepSeek 受控 smoke**（`app/cli/smoke_valuation_analysis.py`）：seed 真实 HTML 链 → target PE=30 / peers 18·20·22 → comparison（premium +50%）→ `ValuationAnalysisService.analyze` 走生产适配器 → 校验 schema / direction / statement / fingerprint → 打印摘要 → **清理全部 seed 数据并实际查询 0 残留**。实跑：2 次真实调用（第一次完整 pipeline 但打印 bug 退出码 1；移除 print 后重跑确认 exit 0），model=`deepseek:deepseek-v4-flash`、assessment=relative_high、assessment_matches_expected=true、cleanup_success=true。
- **边界**：不做 4D Claim Synthesis / LangGraph integration / Report / Audit / HTTP API / Chroma / Retrieval / DCF / 绝对公允价值 / target price / 买卖建议；一个 request 最多 1 个 Valuation Claim；不接 additional Evidence；不修改 generic / Financial / Macro 既有 schema；**不把 `RelativeValuationComparison` 伪装成 EvidenceCard**。
- **决策记录**：[docs/decisions/0035-structured-relative-valuation-analysis.md](docs/decisions/0035-structured-relative-valuation-analysis.md)。

## Claim Synthesis Input & Provenance Foundation（阶段 4D.1A）

阶段 4D.1A 建立 **Claim 综合的输入集边界**：把调用方**显式选出的 2..50 条跨 domain Claim + company + research_question + analysis_as_of** 登记为一个不可变 `SynthesisRun`，并校验每条输入 Claim 的**完整性 / 隔离 / no-lookahead**，使未来 LangGraph 合成节点只能消费"已验证、同公司、同问题、不掺未来信息"的 Claim 集合。**不调用 LLM、不接 LangGraph 合成节点、不做语义筛选**（合成判断属于 4D.1B）。**状态（2026-08-10）：implementation completed / automated tests completed / Windows + Docker runtime acceptance completed——4D.1A = completed**。

- **Migration 0028**（`alembic current` = 0028 head）：`claim_synthesis_runs`（synthesis_id PK、company_id FK RESTRICT、research_question + research_question_sha256、analysis_as_of、synthesis_schema_version >= 1、synthesis_fingerprint CHAR(64) **UNIQUE**、created_at）+ `claim_synthesis_input_links`（PK(synthesis_id, claim_id)、synthesis_id FK CASCADE、claim_id FK claims **RESTRICT**）。**不复制** Evidence / Calculation / Transmission / Comparison ID 到 synthesis 表（Claim → 各 domain 子表 → Evidence → Source 的 provenance 已在既有 schema 中）。**downgrade guard**：任一表有行 → 拒绝回滚（不静默丢弃已登记的 synthesis 输入集），alembic_version 保持 0028。
...
- **ClaimIntegrityGateway（spec N）**：按 Claim 的**真实 analysis_domain + claim_schema_version** dispatch 到 generic（business / event / risk v1）/ Financial（v2/v3）/ Macro（v4/v5/v6）/ Valuation（v7）完整性校验——从 persisted links + domain 子表重建 fingerprint 输入 → 调用各 domain **公开** `compute_*_fingerprint` → 对比 claim.claim_fingerprint（唯一能处理含 automatic Evidence 的历史 Claim 的方案；**禁止复制** domain formula / policy 逻辑）。缺失 / 损坏 → `SynthesisClaimIntegrityError`，**不自动 repair**；legacy macro v1/v2 链无 analysis_as_of → `SynthesisUnsupportedClaimSchema`。
- **隔离 / temporal（no-lookahead）**：research-question 隔离（每个 claim.research_question_sha256 == draft hash → `SynthesisResearchQuestionMismatch`）；company 隔离（→ `SynthesisCompanyMismatch`）；evidence availability（复用 `resolve_availability`，document → published_at 否则 acquired_at、macro → snapshot.fetched_at，**绝不用 reporting_period_end**）<= analysis_as_of，future → `SynthesisFutureEvidence`；无法解析 → `SynthesisTemporalEvidenceInsufficient`；domain_analysis_as_of（macro chain / valuation profile）也必须 <= cutoff。
- **Fingerprint / persistence / replay**：`compute_synthesis_fingerprint` = canonical JSON + SHA-256（含 schema version / company / question / sha256 / cutoff / claims 按 claim_id 排序；**不含 synthesis_id / created_at**）；两步提交（短 session 校验 → 关闭 → 纯函数派生 → 短 transaction `create_or_get`，PG `ON CONFLICT(synthesis_fingerprint)` 并发去重，0 partial write）；fingerprint 命中 → 完整重放校验（run 字段 + exact claim set + 重新执行全部校验），损坏 → `SynthesisIntegrityError`，**不自动 repair**。**并发相同 fingerprint → 最终 1 run + 1 套 links**。
- **测试（54 项新增）**：**25 项单元**（`tests/synthesis/test_contracts.py`，零 DB）+ **26 项集成**（`tests/integration/test_claim_synthesis_service.py`，真实 PG + 真实服务链：gateway dispatch 全 domain + missing / unsupported / corrupted、isolation、temporal、persistence / replay / 并发 / corruption / no mutation、cross-domain E2E、boundary）+ **3 项 migration 0028 downgrade guard**（`tests/integration/test_migration_0028_downgrade_guard.py`，isolated 临时 PG）。全量 **1694 非集成 + 639 集成通过**。全程 0 真实 LLM / 0 Chroma / 0 LangGraph / 0 Report 表。
- **Runtime acceptance（2026-08-10）**：Windows + Docker 各 live/ready **5×200**、ready 五项 checks（configuration / database / chroma / checkpoint / raw_storage）全部 ok；Docker runtime `alembic_version=0028` + 两张 synthesis 表可读。
- **边界**：不创建 Report / DraftSection / Report / ReviewIssue / Audit；不接 LangGraph 合成节点；不开放 HTTP API；不做语义筛选 / 冲突检测 / 证据缺口（4D.1B 及以后）；**不开始 4D.1B**。
- **决策记录**：[docs/decisions/0036-claim-synthesis-input-foundation.md](docs/decisions/0036-claim-synthesis-input-foundation.md)。
