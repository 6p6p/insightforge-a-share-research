...
- **边界**：不创建 Report / DraftSection / ReviewIssue / Audit；不接 LangGraph 分析节点；不改动 generic Claim schema；不批量 update 历史 v4/v1 rows；不引入 transmission ↔ claim join table；**不开始 4C.1B**（Structured Macro Context Analyst）。
