...
只负责后台调度 `ResearchOrchestrationRunner.run_orchestration`（参考
`WorkflowExecutionManager` 的 asyncio registry 模式，**不复制 Stage1 simulation /
resume/cancel 的 WorkflowRun 逻辑**——child / action 语义仍由
`ResearchOrchestrationService` + Stage4/5 runners 承担）。

- `schedule(orchestration_id)`：同 orchestration_id 至多一个 background task
  （已调度未完成 → no-op False）；task done → 自动从 registry 清除；
...
class ResearchOrchestrationExecutionManager:
    ...
    def schedule(self, orchestration_id: UUID) -> bool:
        ...
        new_task = asyncio.create_task(
            self._run(orchestration_id), name=f"orchestration-{orchestration_id}"
        )
    ...
    async def _run(self, orchestration_id: UUID) -> None:
        """执行顶层 run；异常由 Runner 投影 PG 终态，本层只安全日志。"""
        try:
