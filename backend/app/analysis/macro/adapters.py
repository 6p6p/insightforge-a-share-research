"""Production LLM adapter (stage 4C.1B): ChatDeepSeek → MacroAnalysisModel。
...
- **显式关闭 thinking**（`extra_body={"thinking": {"type": "disabled"}}`）：
  DeepSeek V4 Flash 默认开启 thinking，但结构化 Macro 分析需要稳定、低成本的
  受约束输出且不产生 `reasoning_content`；`temperature=0` 不等于关闭 thinking，
  必须显式传参（`thinking` 非标准 OpenAI 参数，按 langchain-deepseek==1.1.0
  公开接口经 `extra_body` 传递）；
- 只启用 structured-output 机制，**不绑定 tools / 不开 web search**（非 agentic）；
- 异常映射：provider / API / 网络异常 → `MacroAnalysisModelUnavailable`；
  输出无法解析为 `MacroAnalysisDecision` → `MacroAnalysisMalformedOutput`；
- **不泄露** raw provider response / key / 完整 prompt / reasoning_content。

自动测试仍用 `FakeMacroAnalysisModel`；真实调用只用于受控 smoke。
"""
...
class DeepSeekMacroAnalysisModel:
    """把官方 `langchain_deepseek.ChatDeepSeek` 包装为 MacroAnalysisModel。
    ...
    async def analyze(
        ...
        try:
            ...
            raise MacroAnalysisModelUnavailable("LLM structured-output 调用失败") from exc
