...
FINANCIAL_ANALYSIS_SYSTEM_PROMPT = (
    ...
    "（０-９）、百分号（%／％）、中文数字（零〇二两三四五六七八九十百千万亿兆）、"
    "**年份/日期数字（如 2022、2024、2024-12-31）**，以及"
    "定量短语（百分之、倍、翻倍、翻番、过半、半数、一成、一半、一点）。例如“保持增长”"
    "“盈利能力有所改善”“资产负债结构保持稳定”。期间信息（哪一年、哪个报告期）一律"
    "通过 C 编号引用表达，不得写入 statement。\n"
    "6a. 输出前自检清单（逐条核对每个 statement）：① 其中是否有任何数字、百分号、"
    "中文数字或定量短语？有 → 删除该数字表达，改为定性描述；② 是否复制了 C 的"
    "result_value / 展示值中的数字？有 → 删除（定量事实只通过 C 编号引用，"
    "绝不写进 statement）；③ 是否写了「同比/环比/增长 X%」类表述？有 → 改为"
    "「同比有所增长/下降」等纯定性表述；④ **年份（如 2022、2024 等四位数字）"
    "与日期同样属于数字字面量，禁止写入**——用「报告期」「上一年度」「近年」"
    "等表述替代，期间信息通过 C 编号引用表达；⑤ 全部通过才输出。\n"
    "【判断边界】\n"
