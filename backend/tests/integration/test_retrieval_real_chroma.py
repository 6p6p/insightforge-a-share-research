"""RetrievalService with a real Chroma backend (stage 3B.2, spec B.10).

需要真实 PostgreSQL（127.0.0.1:5433）+ 真实 Chroma（127.0.0.1:8002）。
索引构建用 FakeEmbeddingProvider（确定性），真实 Chroma 独立测试 collection
（uuid 后缀），测试结束删除。验证（最低集）：
- 索引 → 检索 全链路：Chroma 过滤 + PG hydrate；
- company filter / chunk_set_id $in 在真实 Chroma 上生效（company 隔离）；
- distance 排序（升序）在真实 Chroma 上保持；
- 测试 collection 删除于 finally，不触碰共享 collection。

不下载真实 BGE 模型；不实现 reranker / threshold / LLM judge。
"""
...
_XINHUA_URL = "https://www.xinhuanet.com/2026/0808/0005.htm"
_SOURCE_TITLE = "新闻标题"
...
async def test_real_chroma_retrieve_company_filter_ordering_and_hydration(
    env, chroma_manager
) -> None:
    chunk_set_id = await _seed_and_chunk(env)
    collection_name = f"test_retrieval_{uuid4().hex[:12]}"
    ...
    try:
        result = await index_service.index_chunk_set(chunk_set_id)
        ...
        # PG hydrate：命中都来自本 company，证据链字段完整。
        assert len(hits) == 3
        for hit in hits:
        ...
        # 真实 Chroma cosine distance 排序：升序（相似度降序）。
        distances = [hit.distance for hit in hits]
        assert distances == sorted(distances)

        # company filter / chunk_set_id $in 在真实 Chroma 上生效：
        # 不存在其他 company 的 record，因此换个 company 查询应只返回空（无 eligible）。
        ...
        with pytest.raises(RetrievalIndexNotReady):
            await retrieval_service.retrieve(
                RetrievalQuery(company_id=uuid4(), query_text="净利润增长", top_k=5)
            )
    finally:
