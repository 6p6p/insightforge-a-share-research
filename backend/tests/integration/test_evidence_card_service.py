...
async def test_create_card_creates_no_stage5_report_tables(env) -> None:
    ...
    未来阶段（5E+）表不得存在；Stage 5A-5D 表（report_outlines / draft_sections
    / reports / report_check_results / report_audits / review_issues，migration
    0032-0035）已存在但本阶段不写行。Stage 4 claims 表由 Stage 4A 单独引入，
    不在这里约束（精确阶段边界名，避免以后过期）。
    """
    _, _, _, chunks = await _seed_html_source(env)
    chunk = chunks[0]
    await _create_card(env, chunk)
