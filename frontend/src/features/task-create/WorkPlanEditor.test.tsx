...
describe('WorkPlanEditor 折叠（Stage 6B.1）', () => {
  it('默认全展开：TextArea 输入可见且可交互', () => {
    const onChange = vi.fn();
    renderWithProviders(<WorkPlanEditor value={items} onChange={onChange} />);
    const textarea = screen.getByLabelText('工作项 1 evidence_card_ids');
  ...
  it('点击折叠头收起 → 内容不可见', async () => {
    ...
    const textarea = screen.getByLabelText('工作项 1 evidence_card_ids');
    expect(textarea).toBeVisible();
    fireEvent.click(screen.getByText('工作项 1'));
  ...
  it('删除按钮位于折叠头 extra，点击删除整项', () => {
    const onChange = vi.fn();
    renderWithProviders(<WorkPlanEditor value={items} onChange={onChange} />);
    fireEvent.click(screen.getByRole('button', { name: '删除工作项 1' }));
