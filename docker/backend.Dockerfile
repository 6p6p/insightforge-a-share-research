...
# BGE 模型缓存（V1.1 closure）：
# - 构建期**尽力**下载 immutable revision 模型（HF 可达时镜像自包含；
#   不可达时构建不失败——compose 以 HF_CACHE_HOST bind mount 提供宿主机缓存，
#   运行时 HF_HUB_OFFLINE=1 离线加载，不依赖 huggingface.co 连通性）。
ENV HF_HOME=/home/appuser/.cache/huggingface
RUN python - <<'PY' || echo "BGE download skipped (offline build); runtime uses HF_CACHE_HOST bind mount"
import time
